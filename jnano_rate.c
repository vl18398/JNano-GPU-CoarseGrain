 ...
 * for setting and reading Jetson NANO's GPU and EMC frequncies.
 */
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
 ...
 */
static void jnano_set_gpu_rate(const unsigned long gpu_rate)
{
    int val = 1;
    FILE *fp[2];
 ...
 * @param emc_rate See ::jnano_emc_freqs
 */
static void jnano_set_emc_rate(const unsigned long emc_rate)
{
    int val = 1;
    FILE *fp[2];
 ...
 */
static void jnano_set_cpu_rate(const unsigned long cpu_rate, const unsigned int cpu)
{
    FILE *fp[2];
...
void jnano_set_rate(const jnano_unit unit, const unsigned long rate)
{
    switch (unit) {
    case EMC_RATE: {
	jnano_set_emc_rate(rate);
	break;
    }
    case GPU_RATE: {
	jnano_set_gpu_rate(rate);
	break;
    }
    case CPU0_RATE: {
	jnano_set_cpu_rate(rate, 0);
	break;
    }
    case CPU1_RATE: {
	jnano_set_cpu_rate(rate, 1);
	break;
    }
    case CPU2_RATE: {
	jnano_set_cpu_rate(rate, 2);
	break;
    }
    case CPU3_RATE: {
	jnano_set_cpu_rate(rate, 3);
	break;
    }
    default:
	break;
