 ...
 * @param file e.g., located under path "/dev/i2c-%d" where "%d" is 1
 * @param reg e.g., vbusReg[rail] where rail is indexed by ::jnano_rail
 */
 ...
 * @param r e.g., rshunt[rail] where rail is indexed by ::jnano_rail
 */
 ...
 * @param measure Either VOLTAGE, POWER or CURRENT. See ::jnanopow_ina3321_measure
 * @param *val Output's reference
 */
static void jnano_get_ina3221_sysf(jnano_rail rail,
				  jnano_rail_type measure,
				  unsigned int *val)
{
  FILE *fp;
  int addr;
 ...
 * @param *val Output's reference
 */
static int jnano_get_ina3221_userspace_i2c(int rail, jnano_rail_type measure, unsigned int *val)
{
  int file;
  int adapter_nr = 1;
  char filename[20];
		      ...
		      unsigned int *val)
{
  ...
  if (rail >= 0 && rail <= 2) {
    jnano_get_ina3221_userspace_i2c(rail, measure, val);
  } else if (rail >= 3 && rail <= 8) {
    jnano_get_ina3221_sysf(rail, measure, val);
  } else {}
